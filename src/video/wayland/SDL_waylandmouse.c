...
static int
create_buffer_from_shm(Wayland_CursorData *d, int width, int height, uint32_t format)
{
    ...
    struct wl_shm_pool *shm_pool = wl_shm_create_pool(data->shm, shm_fd, size);
...
Wayland_CreateCursor(SDL_Surface *surface, int hot_x, int hot_y)
{
    ...
    if (cursor) {
        ...
        if (create_buffer_from_shm (data, surface->w, surface->h, WL_SHM_FORMAT_XRGB8888) < 0)
        {
        ...
        SDL_memcpy(data->shm_data, surface->pixels, surface->h * surface->pitch);

        data->surface = wl_compositor_create_surface(wd->compositor);
        wl_surface_attach(data->surface, data->buffer, 0, 0);
        wl_surface_damage(data->surface, 0, 0, surface->w, surface->h);
...
CreateCursorFromWlCursor(SDL_WaylandData *d, struct wl_cursor *wlcursor)
{
    ...
    if (cursor) {
        ...
        wl_surface_attach(data->surface, wl_cursor_image_get_buffer(wlcursor->images[0]), 0, 0);
        wl_surface_damage(data->surface, 0, 0, wlcursor->images[0]->width, wlcursor->images[0]->height);
...
Wayland_CreateDefaultCursor()
{
    ...
    return CreateCursorFromWlCursor (data, wl_cursor_theme_get_cursor(data->cursor_theme, "left_ptr"));
}
...
Wayland_CreateSystemCursor(SDL_SystemCursor id)
{
    ...
    default:
        SDL_assert(0);
        return NULL;
    case SDL_SYSTEM_CURSOR_ARROW:     cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "left_ptr");
    case SDL_SYSTEM_CURSOR_IBEAM:     cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "xterm");
    case SDL_SYSTEM_CURSOR_WAIT:      cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "wait");
    case SDL_SYSTEM_CURSOR_CROSSHAIR: cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "hand1");
    case SDL_SYSTEM_CURSOR_WAITARROW: cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "wait");
    case SDL_SYSTEM_CURSOR_SIZENWSE:  cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "hand1");
    case SDL_SYSTEM_CURSOR_SIZENESW:  cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "hand1");
    case SDL_SYSTEM_CURSOR_SIZEWE:    cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "hand1");
    case SDL_SYSTEM_CURSOR_SIZENS:    cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "hand1");
    case SDL_SYSTEM_CURSOR_SIZEALL:   cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "hand1");
    case SDL_SYSTEM_CURSOR_NO:        cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "xterm");
    case SDL_SYSTEM_CURSOR_HAND:      cursor = wl_cursor_theme_get_cursor(d->cursor_theme, "hand1");
    }
